...
# ==========================================
@app.route('/srpcom/list-accounts/<protocol>', methods=['GET'])
...
@app.route('/srpcom/monitor-xray', methods=['GET'])
def monitor_xray():
    try:
        ip_data = {}
                    ...
                    if 'accepted' in line and 'email:' in line:
                        ...
                        for idx, token in enumerate(parts):
                            if token == 'from' and idx + 1 < len(parts):
                                raw = parts[idx + 1].replace('tcp:', '').replace('udp:', '')
                                ip = raw.split(':')[0]
...
@app.route('/srpcom/monitor-ssh', methods=['GET'])
def monitor_ssh():
    try:
        res = "💻 *SSH MONITORING*\n━━━━━━━━━━━━━━━━━━━━\n"
...
@app.route('/srpcom/log-<logtype>', methods=['GET'])
...
@app.route('/srpcom/speedtest', methods=['GET'])
def run_speedtest():
    try:
...
@app.route('/srpcom/bandwidth', methods=['GET'])
def run_bandwidth():
    try:
...
def generate_account_detail_grpc(protocol, user, uid, exp_date_str, is_trial=False, limit_ip=0, limit_quota=0):
    trial_txt = " TRIAL" if is_trial else ""
    header_txt = "Pembuatan akun trial berhasil" if is_trial else "Pembuatan akun baru berhasil"
    ...
    if protocol == 'vmess':
        tls_dict = {"v":"2","ps":user,"add":DOMAIN,"port":"443","id":uid,"aid":"0","net":"grpc","type":"none","host":DOMAIN,"path":"vmessgrpc","tls":"tls","sni":DOMAIN}
        none_tls_dict = {"v":"2","ps":user,"add":DOMAIN,"port":"80","id":uid,"aid":"0","net":"grpc","type":"none","host":DOMAIN,"path":"vmessgrpc","tls":"","sni":""}
        link_tls = "vmess://" + base64.b64encode(json.dumps(tls_dict, separators=(',', ':')).encode('utf-8')).decode('utf-8')
        link_none = "vmess://" + base64.b64encode(json.dumps(none_tls_dict, separators=(',', ':')).encode('utf-8')).decode('utf-8')
    elif protocol == 'vless':
        link_tls = f"vless://{uid}@{DOMAIN}:443?mode=gun&security=tls&encryption=none&host={DOMAIN}&type=grpc&serviceName=vlessgrpc&sni={DOMAIN}#{user}"
        link_none = f"vless://{uid}@{DOMAIN}:80?mode=gun&security=none&encryption=none&host={DOMAIN}&type=grpc&serviceName=vlessgrpc#{user}"
    elif protocol == 'trojan':
        link_tls = f"trojan://{uid}@{DOMAIN}:443?mode=gun&security=tls&host={DOMAIN}&type=grpc&serviceName=trojangrpc&sni={DOMAIN}#{user}"
        link_none = f"trojan://{uid}@{DOMAIN}:80?mode=gun&security=none&type=grpc&serviceName=trojangrpc#{user}"

    msg_cli = (
        f"{header_txt}\n\n━━━━━━━━━━━━━━━━━━━━\n❖ XRAY/{protocol.upper()} GRPC{trial_txt} ❖\n━━━━━━━━━━━━━━━━━━━━\n"
        f"Remarks : {user}\nIP Address : {IP_ADD}\nDomain : {DOMAIN}\n"
        f"Port TLS : 443\nPort NONE-TLS : 80\n{'Password' if protocol == 'trojan' else 'ID'} : {uid}\n"
        f"Network : gRPC\nService Name : {protocol}grpc\n━━━━━━━━━━━━━━━━━━━━\n"
        f"Limit IP : {lim_ip_str}\nLimit Kuota : {lim_q_str}\n━━━━━━━━━━━━━━━━━━━━\n"
        f"LINK GRPC TLS : {link_tls}\n━━━━━━━━━━━━━━━━━━━━\n"
        f"LINK GRPC NONE-TLS : {link_none}\n"
        f"━━━━━━━━━━━━━━━━━━━━\nExpired On : {exp_date_str} WIB"
    )
...
@app.route('/srpcom/add-<protocol>grpc', methods=['POST'])
def add_user_grpc(protocol):
    data = request.json or {}
    user, exp = data.get('user'), int(data.get('exp', 30))
    limit_ip, limit_quota = int(data.get('limit_ip', 0)), int(data.get('limit_quota', 0))
    if not user: return jsonify({"stdout": "Error: User required"}), 400
    ...
    msg_cli, msg_tg = generate_account_detail_grpc(protocol, user, uid, dt_str, False, limit_ip, limit_quota)
    send_telegram(msg_tg)
    return jsonify({"stdout": msg_cli, "stdout_tg": msg_tg})


@app.route('/srpcom/trial-<protocol>grpc', methods=['POST'])
def trial_user_grpc(protocol):
    data = request.json or {}
    exp_min = 60 
    limit_ip = int(data.get('limit_ip', 1))
    ...
    msg_cli, msg_tg = generate_account_detail_grpc(protocol, user, uid, dt_str, True, limit_ip, 1)
    send_telegram(msg_tg)
    return jsonify({"stdout": msg_cli, "stdout_tg": msg_tg})


@app.route('/srpcom/detail-<protocol>grpc', methods=['GET', 'POST'])
def detail_user_grpc(protocol):
    user = (request.json or {}).get('user')
    cfg = load_json(XRAY_CONF)
    uid = None
    ...
    if uid:
        ...
        msg_cli, msg_tg = generate_account_detail_grpc(protocol, user, uid, exp_date_str, False, limit_ip, limit_q)
...
@app.route('/srpcom/del-<protocol>grpc', methods=['DELETE', 'POST'])
def del_user_grpc(protocol):
    user = (request.json or {}).get('user')
    if not user: return jsonify({"stdout": "Error: User required"}), 400
    cfg = load_json(XRAY_CONF)
    found = False
...
@app.route('/srpcom/renew-<protocol>grpc', methods=['POST'])
def renew_user_grpc(protocol):
    user, exp = (request.json or {}).get('user'), int((request.json or {}).get('exp', 30))
    if not user: return jsonify({"stdout": "Error: User required"}), 400
    dt_str = "Error"
    found = False
...
# SSH ENDPOINTS
# ==========================================
@app.route('/srpcom/add-ssh', methods=['POST'])
def add_ssh():
    data = request.json or {}
...
@app.route('/srpcom/del-ssh', methods=['DELETE', 'POST'])
def del_ssh():
    user = (request.json or {}).get('user')
    if not user: return jsonify({"stdout": "Error: User required"}), 400
...
@app.route('/srpcom/renew-ssh', methods=['POST'])
def renew_ssh():
    user, exp = (request.json or {}).get('user'), int((request.json or {}).get('exp', 30))
    if not user: return jsonify({"stdout": "Error: User required"}), 400
...
@app.route('/srpcom/detail-ssh', methods=['GET', 'POST'])
def detail_ssh():
    user = (request.json or {}).get('user')
...
@app.route('/srpcom/trial-ssh', methods=['POST'])
def trial_ssh():
    data = request.json or {}
    exp_min = 60 
    limit_ip = int(data.get('limit_ip', 1))
...
# L2TP ENDPOINTS
# ==========================================
@app.route('/srpcom/add-l2tp', methods=['POST'])
def add_l2tp():
    data = request.json or {}
    user, password, exp = data.get('user'), data.get('password', '123'), int(data.get('exp', 30))
    if not user: return jsonify({"stdout": "Error: User required"}), 400
...
@app.route('/srpcom/del-l2tp', methods=['DELETE', 'POST'])
def del_l2tp():
    user = (request.json or {}).get('user')
    if not user: return jsonify({"stdout": "Error: User required"}), 400
...
@app.route('/srpcom/renew-l2tp', methods=['POST'])
def renew_l2tp():
    user, exp = (request.json or {}).get('user'), int((request.json or {}).get('exp', 30))
    if not user: return jsonify({"stdout": "Error: User required"}), 400
...
@app.route('/srpcom/detail-l2tp', methods=['GET', 'POST'])
def detail_l2tp():
    user = (request.json or {}).get('user')
...
@app.route('/srpcom/trial-l2tp', methods=['POST'])
def trial_l2tp():
