...
# Menyimpan Masa Aktif Lisensi ke file lokal agar bisa dibaca oleh Menu
mkdir -p /usr/local/etc/srpcom
EXP_DATE=$(echo "$JSON_BODY" | grep -o '"expires_at":"[^"]*' | cut -d'"' -f4)
echo "$EXP_DATE" > /usr/local/etc/srpcom/license_exp.txt
...
echo -e "\n[10.5/11] Menyiapkan Sistem Pengecekan Lisensi Otomatis..."
cat > /usr/local/bin/srpcom/license_check.sh << 'EOF'
#!/bin/bash
...
if [ "$HTTP_STATUS" != "200" ]; then
    # Jika Lisensi Expired / Dihapus -> Matikan Semua Layanan VPN
    systemctl stop xray caddy dropbear ssh-ws xl2tpd ipsec openvpn-server@server-udp openvpn-server@server-tcp badvpn-7100 badvpn-7200 badvpn-7300 xray-api srpcom-bot
    echo "EXPIRED" > /usr/local/etc/srpcom/license_exp.txt
else
    # Update masa aktif terbaru
    EXP_DATE=$(echo "$JSON_BODY" | grep -o '"expires_at":"[^"]*' | cut -d'"' -f4)
    if [ -n "$EXP_DATE" ]; then
        echo "$EXP_DATE" > /usr/local/etc/srpcom/license_exp.txt
    fi

    # Jika Lisensi Aktif/Diperpanjang tapi layanan mati -> Nyalakan kembali
    systemctl is-active --quiet xray || systemctl start xray caddy dropbear ssh-ws xl2tpd ipsec openvpn-server@server-udp openvpn-server@server-tcp badvpn-7100 badvpn-7200 badvpn-7300 xray-api srpcom-bot
fi
EOF
chmod +x /usr/local/bin/srpcom/license_check.sh

echo -e "\n[11/11] Setup Cronjob Selesai..."
if ! grep -q "menu" /root/.profile; then echo "menu" >> /root/.profile; fi

# Menambahkan cronjob untuk Auto Expired VPS (Berjalan setiap jam ke-0)
echo "0 * * * * root /usr/local/bin/srpcom/auto_expired.sh >/dev/null 2>&1" > /etc/cron.d/auto_expired

# Menambahkan cronjob untuk Cek Lisensi Script (Berjalan setiap 6 jam)
echo "0 */6 * * * root /usr/local/bin/srpcom/license_check.sh >/dev/null 2>&1" > /etc/cron.d/license_check
